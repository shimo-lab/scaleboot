...
\def\documentid{\getTime-stamp: "2006-10-30 08:54:29 shimo"}
...
The step (iii) is performed by the the {\tt summary} method as
already mentioned. The first line shows the raw BP
$\alpha_1$.  The main results are the corrected $p$-values follow
next. For each model, we calculate $q_k$, $k=1,2,3$, by
\[
...
trees.  {\tt mam15.cnt}: multiscale bootstrap counts.  {\tt
mam15.lnf}: site-wise log-likelihood values (output from PAML).  {\tt
mam15.log}: detailed information for the associations.  {\tt
mam15.mt}: site-wise log-likelihood values (output from seqmt).  {\tt
...
First we sort the results in decreasing order of the likelihood values,
<<prepmam,echo=false>>=
library(scaleboot)
data(mam15)
mam15.trees <- mam15.relltest[1:15]
