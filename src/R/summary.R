...
##                 "S3 generic/method consistency ... WARNING"
  ...
  if(is.null(object$fi)) {
    ...
    object$best <- list(model="raw",aic=0,pv=pv,pe=pe,betapar=NULL)
    object$average <- list(model="raw",w=structure(1,names="raw"),pv=pv,pe=pe,betapar=NULL)
    return(object)
  }
...
#  object$best <- list(model=model,aic=aic0,pv=pvbest,pe=pebest,dv=dv[[i]],de=de[[i]])
  object$best <- list(model=model,aic=aic0,pv=pvbest,pe=pebest,betapar=betapar[[i]])
...
#  tab <- cbind(pval,beta0,aic,weight) # to be catmat
  tab <- cbind(pval,beta,aic,weight) # to be catmat
...
#  catmat(cbind(pvalbest,beta0))
  catmat(cbind(pvalbest, betabest))
  cat("\n")
...
print.summary.scalebootv <- function(x,select="average",sort.by=NULL,...) {
  ...
  out <- matrix("",length(x),1+length(pvalues)+2,
              dimnames=list(names(x),c("raw",pvalues,"beta0","beta1")))

  ## numerical values for sorting
  outval <- matrix(0,length(x),1+length(pvalues),dimnames=list(names(x),c("raw",pvalues)))
